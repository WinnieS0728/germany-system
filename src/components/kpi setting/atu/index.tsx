...
type defaultValue = { atu: kpiInput[] };
...
export default function AtuSettingTable() {
  const { t } = useTranslation(["kpiSetting"]);
  const salesList = useAppSelector((state) => state.salesList).body;
  const { thisYear } = useTime();
  const { GetThreshold, PostThreshold } = useKpiSetting("atu");
  ...
  async function checkDataExist(EmpId: string): Promise<boolean> {
    const res = await api.getKpi({ year: thisYear, EmpId, type: "atu" });
    if (res.length > 0) {
      return true;
    } else {
      return false;
